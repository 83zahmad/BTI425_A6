...
export default function ArtworkCardDetail({ objectID }) {
  ...
  // Determine if this objectID is in the user's favourites
  useEffect(() => {
    setShowAdded(favouritesList?.includes(objectID));
  }, [favouritesList, objectID]);

  // Handle adding/removing favourites via secured API
  async function handleFavouriteClick() {
    ...
    setFavouritesList(updatedList); // Update atom with new list from API
  }
